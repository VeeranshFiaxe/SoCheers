...
(function () {
  ...
  const grids = new Map();
  function initGrids() {
    document.querySelectorAll("[data-pixgrid]").forEach((host) => {
      const cols = parseInt(host.dataset.cols || 20, 10);
      const rows = parseInt(host.dataset.rows || 8, 10);
      ...
      grids.set(host, { tiles, cols, rows });
    });
  }
  ...
  function initRollText() {
    document.querySelectorAll("[data-roll]").forEach((el) => {
      if (el.querySelector(".roll__in")) return;
      const txt = el.textContent.trim();
      const inner = document.createElement("span");
      inner.className = "roll__in";
      const a = document.createElement("span"); a.textContent = txt;
      const b = document.createElement("span"); b.textContent = txt; b.setAttribute("aria-hidden", "true");
      inner.append(a, b);
      el.textContent = "";
  ...
  function runLoader() {
    ...
    if (!loader) return;
    if (!hasGsap || prefersReduced) { loader.style.display = "none"; return; }
    ...
    tl.to(state, {
      ...
      onUpdate: () => { if (countEl) countEl.textContent = Math.round(state.v); },
    });
  ...
  function initHero() {
    ...
    const crop = document.querySelector("[data-stage-crop]");
    const photo = document.querySelector("[data-stage-img]");
    const cards = gsap.utils.toArray("[data-scard]");
    const num = document.querySelector("[data-deck-num]");
    if (!hero || !pin || !frame || !stage || !hasGsap) return;
    ...
    gsap.set(cards, { yPercent: 125, rotate: (i) => (i % 2 ? 9 : -9), autoAlpha: 0 });
  ...
  function initSplits() {
    ...
    document.querySelectorAll("[data-split]").forEach((el) => {
      const build = () => {
        if (prefersReduced || !window.SplitText) { gsap.set(el, { autoAlpha: 1 }); return; }
  ...
  function initReveals() {
    ...
    document.querySelectorAll("[data-clip]").forEach((el, i) => {
      gsap.fromTo(el,
  ...
  /* -------------------------------------------------- marquees */
  let _marqueesReady = false;
  function initMarquees() {
    if (!hasGsap || _marqueesReady) { initMarquees.reflow && initMarquees.reflow(); return; }
    _marqueesReady = true;
    const items = [];
    document.querySelectorAll("[data-marquee]").forEach((track) => {
      ...
      items.push(state);
      if (lenis) lenis.on("scroll", (e) => { state.boost += gsap.utils.clamp(-30, 30, (e.velocity || 0) * dir); });
    });
    ...
    initMarquees.reflow = () => items.forEach((s) => { s.half = s.track.scrollWidth / 2; });
    window.addEventListener("resize", () => initMarquees.reflow());
  }
  ...
  function initRail() {
    ...
    const setActive = (i) => items.forEach((it, n) => it.classList.toggle("is-active", n === i));
    document.querySelectorAll("[data-sec]").forEach((sec) => {
      const i = parseInt(sec.dataset.sec, 10);
      ScrollTrigger.create({
  ...
  function initCursor() {
    ...
    if (!cursor || !canHover || !hasGsap) return;
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      if (rxTo) { rxTo(e.clientX); ryTo(e.clientY); }
    });
  ...
  function initSpotlight() {
    ...
    if (!spot || !canHover || !hasGsap || prefersReduced) return;
  ...
  function initTilt() {
    if (!hasGsap || !canHover || prefersReduced) return;
  ...
  function boot() {
    ...
    if (!hasGsap) { const l = document.getElementById("loader"); if (l) l.style.display = "none"; return; }
    ...
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(() => { initMarquees.reflow && initMarquees.reflow(); ScrollTrigger.refresh(); });
    }
  }
