/* ============================================================
   ...
   delivery, not a content model, so this script turns it into one:
     ...
     · films are cut down to what a tile can use, by
       scripts/build-ai-films.mjs, which this calls once per film. They
       are 1.28GB of 1080p masters as delivered and the grid draws them
       600px wide with the sound off; the cuts come to a few percent of
       that, and each one gets a poster beside it so the wall can paint
       without loading any video at all. When Cloudinary is stood up
       (see lib/ai-content.ts) these paths are what changes.
...
import { cutFilm, cutPoster } from "./build-ai-films.mjs";

const ROOT = path.resolve(fileURLToPath(new URL("..", import.meta.url)));
const SRC = path.join(ROOT, "assets/SC Website Revamp/05. AI work");
...
for (const { dir, kind } of FOLDERS) {
  ...
  for (const name of fs.readdirSync(abs).sort()) {
    ...
    if (isImg) {
      ...
      /* The master is read and never served. scripts/build-ai-films.mjs
         writes the copy the tile actually loads - long edge 960, no
         audio, faststart - and pulls its opening frame out as a poster,
         which is what lets components/AiGrid.tsx paint the whole wall
         without putting a single <video> in the tree. Both steps skip
         themselves when the derived file is already newer than the
         master, so a re-run after one new film costs one encode. */
      const cut = cutFilm(file, slug);
      const poster = cut ? await cutPoster(path.join(ROOT, "public", cut), slug) : undefined;
      /* a film ffmpeg will not take is still a film: it falls back to the
         master where it was delivered, which is what every film here did
         before there was an encoder in this repo */
      const url =
        cut ??
        "/assets/" +
          ["SC Website Revamp", "05. AI work", dir, name].map(encodeURIComponent).join("/");
      out.push({ ...asset, w: dim.w, h: dim.h, src: url, poster });
      const mb = (f) => (fs.statSync(f).size / 1048576).toFixed(1);
      process.stdout.write(
        `· ${slug.padEnd(30)} ${dim.w}x${dim.h}  ${mb(file)}MB -> ` +
          `${cut ? mb(path.join(ROOT, "public", cut)) + "MB" : "master (declined)"}\n`,
      );
    }
  }
}
